...
#     tpr,fpr,accuracy,best_thresholds=calculate_accuracy(thresholds,distances,labels)
#     thresholds=np.arange(0,4,0.01)
#     test,test_std,far=calculate_accuracy(thresholds,distances,labels)
#     return test,test_std,far,tpr,fpr,best_thresholds
...
def calculate_roc(thresholds, distances, labels, nrof_folds=5):
    ...
    print(nrof_thresholds,"nrof_thresholds")
    # k_fold = KFold(n_splits=nrof_folds, shuffle=False)

    tprs = np.zeros((1,nrof_thresholds))
    fprs = np.zeros((1,nrof_thresholds))
    accuracy = np.zeros((1))

    indices = np.arange(nrof_pairs)

    for fold_idx, test_set in enumerate(indices):
